...
class Rank(params: MemoryConfigurationParameters, bankParams: DRAMBankParameters) extends PhysicalMemoryModuleBase {
  ...
  for (i <- 0 until params.numberOfBankGroups) {
    ...
    when(reqQueues(i).io.enq.fire) {
      printf(
        p"[Rank] Request enqueued to bankGroup $i: addr=0x${Hexadecimal(io.memCmd.bits.addr)} data=0x${Hexadecimal(io.memCmd.bits.data)}\n"
      )
    }
  }
  ...
  for (i <- 0 until params.numberOfBankGroups) {
    ...
    when(respQueues(i).io.enq.fire) {
      printf(p"[Rank] Response enqueued from bankGroup $i: addr=0x${Hexadecimal(
          groups(i).io.phyResp.bits.addr
        )} data=0x${Hexadecimal(groups(i).io.phyResp.bits.data)}\n")
    }
  }
