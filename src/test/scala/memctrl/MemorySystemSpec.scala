...
class SingleChannelMemorySystemSpec extends AnyFreeSpec with Matchers {
  ...
  "SingleChannelSystem should correctly handle a write followed by a read transaction" in {
    simulate(new SingleChannelSystem(SingleChannelMemoryConfigurationParams(MemoryConfigurationParameters(numberOfRanks = 2, numberOfBanks = 4), DRAMBankParameters()))) { dut =>
