// File: src/test/scala/memctrl/DRAMBankSpec.scala
package memctrl
...
class DRAMBankSpec extends AnyFreeSpec with Matchers {
  "DRAMBank should handle basic DRAM operations via Decoupled I/O" in {
    simulate(new DRAMBank(DRAMBankParameters())) { c =>
      // Bring reset up
      c.clock.step(5)
      ...
      def sendCmd(
        ...
        // One extra cycle to complete the handshake
        c.clock.step()
        // Deassert valid again
        c.io.memCmd.valid.poke(false.B)
      }
      ...
      println("Sending over an ACTIVATE command")
      sendCmd(testAddr, 0.U, cs = false, ras = false, cas = true,  we = true)
      expectResp()  // ACK with default ackData
      ...
      // If we reach here, all scenarios passed
    }
  }
}
